...
use thiserror::Error;
use user_server_sdk::error::{ForbiddenError, SigninError};
...
impl AuthSigner {
    ...
    pub fn sign(&self, data: String, duration: Duration) -> Result<String> {
        let claims = Claims::with_custom_claims(CustomClaims { data }, duration)
